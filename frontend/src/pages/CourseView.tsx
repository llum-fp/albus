...
import { ArrowLeft, CheckCircle, Rocket, BookOpen, Target, Quote } from "lucide-react";
    ...
    <div className="max-w-3xl mx-auto px-4 lg:px-8 py-8 section-enter">
      ...
      <div className="mb-6">
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <span className={`badge badge-sm font-semibold badge-${course.status}`}>
            {course.status}
          </span>
        ...
        <h1 className="text-2xl lg:text-3xl font-black text-base-content tracking-tight leading-snug">
          {course.title}
        </h1>
        {course.summary && (
          <p className="mt-3 text-base-content/60 leading-relaxed">{course.summary}</p>
        )}
      </div>
        ...
        <div className="card-clean bg-amber-50 border-amber-200 p-4 mb-6 flex flex-wrap items-center gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-amber-600/70 mt-0.5">
              El contenido generado es un borrador. Revísalo antes de aprobarlo.
            </p>
          </div>
          {course.status === "draft" && (
            <button
            ...
            {m.citations.length > 0 && (
              <div className="mt-4 pt-4 border-t border-base-200">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-semibold text-base-content/30 uppercase tracking-wider">
                    Fuentes
                  </span>
                </div>
                <ul className="flex flex-col gap-1">
                  {m.citations.map((c, i) => (
                    <li key={i} className="text-xs text-base-content/40 font-medium">
                      {c.title}
                    </li>
                  ))}
                </ul>
